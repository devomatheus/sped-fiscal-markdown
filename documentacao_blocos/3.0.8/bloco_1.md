...
NºCampo Descrição Tipo Tam Dec Obrig
01 REG Texto fixo contendo "1250” C 004 - O
02 VL_CREDITO_ICMS_OPInformar o valor total do ICMS operação própria que o N - 02 O
...
conforme Tabela 5.7
03 VL_CREDITO_ICMS_OP_MOTInformar o valor total do ICMS operação própria que o N - 02 O
informante tem direito ao crédito, na forma prevista na
...
18 COD_ITEM Informar o insumo conforme código do item (campo 02 do C 060 - O
Registro 0200)
...
Campo 19 (TP_RESIDUO) – Informar o tipo de resíduo. Valores válidos: [01, 02, 03].
Observação:
DDG – Dry distillers grain (resíduo seco)
WDG – Wet distillers grain (resíduo úmido)
Campo 20 (QTD_RESIDUO) – Informar a quantidade de resíduo produzido (toneladas) resultante do processo
de produção.
...
02 COD_ITEM_IPM Código do item (Tabela própria da unidade da federação C 060 - O
(Tabela de Itens UF Índice de Participação dos Municípios) ou
...
informado.
Validação: o valor informado deve existir em tabela própria da unidade da federação (Tabela de Itens UF Índice de
Participação dos Municípios) e, caso não exista tal tabela, o valor informado deve existir no campo COD_ITEM do registro
0200.
...
Guia Prático EFD-ICMS/IPI – Versão 3.0.8
Atualização: 26 de novembro de 2021
Este registro tem o objetivo de discriminar os pagamentos realizados ou a realizar, referentes à apuração em separa-
do (sub-apuração) do ICMS identificada no registro 1900. A soma do valor das obrigações deste registro deve ser igual à
soma dos campos VL_ICMS_RECOLHER_OA e DEB_ESP_OA, do registro 1920.
Nº Campo Descrição Tipo Tam Dec Obrig
...
02 ALIQ_IMP_BAS Alíquota incidente sobre as importações-base N - 02 O
E
...
09 G4_07 Saldo devedor do ICMS antes das deduções do incentivo (PI e itens não N - 02 O
incentivados)
