...
Campo 14 (CHV_CTE_REF) - Preenchimento: Quando o campo TP_CT-e ou BP-e se referir a “BP-e Substituição”, informar
a chave do BP-e substituído. Nas demais situações o Campo não deve ser preenchido.
...
02 COD_PART_CONS Código do participante (campo 02 do Registro C 060 - apresentar OC
G 0150):
- consignatário, se houver
Página 181 de 356
...
o dígito do quarto caractere for igual a “3”; “4” ou “5”. Neste caso o valor informado no campo 07 -
...
VL_ICMS gera um ajuste a débito (campo 03- VL_AJ_DEBITOS) no registro E110 e também um outro
lançamento a crédito no registro 1920 (campo 05 - VL_TOT_TRANSF_CRÉDITOS_OA) da apuração
em separado (sub-apuração) que for definida no campo 02 - IND_APUR_ICMS do registro 1900 por
meio dos códigos “3”, “4” ou “5”, que deverá coincidir com o quarto caractere do COD_AJ.
...
combinação de CST_ICMS, CFOP e alíquota
do ICMS.
1 COD_OBS Código da observação do lançamento fiscal C 006 - OC
1 (campo 02 do Registro 0460)
Observações:
Nível hierárquico - 3
Ocorrência – 1:N
...
Nos casos em que a escrituração das notas de saída for definida pela legislação da UF com a escrituração consolidada,
apenas as notas de entrada, de substituição e de ajuste serão escrituradas individualmente.
...
Validação do Registro: não podem ser informados dois ou mais registros com a combinação de mesmos valores dos
campos IND_OPER, IND_EMIT, COD_PART, SER, NUM_DOC e DT_DOC, quando o campo COD_PART for preenchido.
Caso contrário, não podem ser informados dois ou mais registros com a combinação de mesmos valores dos campos
IND_OPER, IND_EMIT, SER, NUM_DOC e DT_DOC.
Nº Campo Descrição Tipo Tam Dec Entr. Saídas
...
4 COD_PART C 60 - O -
Registro 0150) do prestador, no caso de entradas.
...
6 COD_SIT N 002* - O O
a Tabela 4.1.2.
7 SER Série do documento fiscal. N 3 - OC OC
...
Valores cobrados em nome do prestador sem
14 VL_SERV_NT N - 2 OC OC
destaque de ICMS.
15 VL_TERC Valores cobrados em nome de terceiros.
Valor de despesas acessórias indicadas no
16 VL_DA N - 2 OC OC
documento fiscal.
...
2 - Cofaturamento
COD_MOD_DOC_ Código do modelo do documento fiscal
25 N 002* - OC OC
REF referenciado, conforme a Tabela 4.1.1.
26 CHV_DOCe_REF Chave da nota referenciada. N 044* - OC OC
Código de autenticação digital do registro, campo
36 do registro do Arquivo tipo mestre de
27 HASH_DOC_REF C 32 - OC OC
documento fiscal, conforme definido no Convênio
115/2003.
...
07 VL_ICMS Parcela correspondente ao "Valor do ICMS" N - 02 O O
referente à combinação CST_ICMS, CFOP, e
alíquota do ICMS, incluindo o FCP, quando
...
federativas.
Nº Campo Descrição Tipo Tam Dec Entr. Saídas
1 REG Texto fixo contendo "D731" C 004 - O O
2 VL_FCP_OP Valor do Fundo de Combate à Pobreza (FCP) N - 02 O O
...
registro D735, que podem ou não alterar o cálculo do valor do imposto.
...
estornados da Apuração Normal (E110) e transferidos para as sub-apurações constantes do registro 1900 e filhos por meio de
lançamentos de ajustes neste registro. Isto ocorrerá quando:
1. o terceiro caractere do código de ajuste (tabela 5.3) do reg. D737 for igual a “2 – Estorno de Débito” e
o dígito do quarto caractere for igual a “3”; “4” ou “5”. Neste caso o valor informado no campo 07 -
VL_ICMS gera um ajuste a crédito (campo 07- VL_AJ_CREDITOS) no registro E110 e também um
outro lançamento a débito no registro 1920 (campo 02 - VL_TOT_TRANSF_DEBITOS_OA) da
apuração em separado (sub-apuração) definida no campo 02- IND_APUR_ICMS do registro 1900 por
meio dos códigos “3”, “4” ou “5”, que deverá coincidir com o quarto caractere do COD_AJ; e
2. o terceiro caractere do código de ajuste (tabela 5.3) do reg. D737 for igual a “5 – Estorno de Crédito”
e o dígito do quarto caractere for igual a “3”; “4” ou “5”. Neste caso o valor informado no campo 07 -
VL_ICMS gera um ajuste a débito (campo 03- VL_AJ_DEBITOS) no registro E110 e também um outro
lançamento a crédito no registro 1920 (campo 05 - VL_TOT_TRANSF_CRÉDITOS_OA) da apuração
em separado (sub-apuração) que for definida no campo 02 - IND_APUR_ICMS do registro 1900 por
...
que não tenham sido canceladas.
...
Validação do Registro: não podem ser informados dois ou mais registros com a combinação de mesmos valores dos
campos COD_MOD, SER, DT_DOC, IND_PREPAGO, COD_MUN_DEST
Nº Campo Descrição Tipo Tam Dec Entr. Saídas
1 REG Texto fixo contendo "D750" C 004 - - O
Código do modelo do documento fiscal, conforme a - O
2 COD_MOD C 002* -
Tabela 4.1.1
3 SER Série do documento fiscal C 003 - - O
4 DT_DOC Data da emissão dos documentos N 008* - - O
Quantidade de documentos consolidados neste - O
5 QTD_CONS N - -
registro
Forma de pagamento: - O
6 IND_PREPAGO 0 – pré pago N 001 -
1 – pós pago
7 VL_DOC Valor total dos documentos N - 02 - O
8 VL_SERV Valor dos serviços tributados pelo ICMS. N - 02 O O
Valores cobrados em nome do prestador sem
9 VL_SERV_NT N - 002 O O
destaque de ICMS.
...
Campo 07 (VL_DOC) – Preenchimento: corresponde à soma dos campos VL_SERV, VL_SERV_NT, VL_TERC e VL_DA
subtraído do campo VL_DESC.
...
05 VL_OPR Valor da operação correspondente à combinação de N - 02 O O
CST_ICMS, CFOP, e alíquota do ICMS, incluídas as
despesas acessórias menos os descontos
incondicionais.
06 VL_BC_ICMS Parcela correspondente ao "Valor da base de cálculo N - 02 O O
do ICMS" referente à combinação CST_ICMS,
CFOP, e alíquota do ICMS
...
federativas.
Nº Campo Descrição Tipo Tam Dec Entr. Saídas
1 REG Texto fixo contendo "D761" C 004 - O O
2 VL_FCP_OP Valor do Fundo de Combate à Pobreza (FCP) N - 02 O O
vinculado à operação própria, na combinação de
CST_ICMS, CFOP e alíquota do ICMS
Observações:
