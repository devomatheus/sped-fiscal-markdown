...
COD_SIT, SER, SUB, NUM_DOC, CHV_CTE e DT_DOC são obrigatórios. Os demais campos são facultativos (se forem
...
Campo 05 - Valores válidos: [07, 08, 8B, 09, 10, 11, 26, 27 e 57] - – Ver tabela reproduzida na subseção 6.4 deste guia.
...
igual a “57” de emissão própria ou de terceiros. A chave de acesso do CT-e passa a ser informação obrigatória para todos
os documentos a partir de abril de 2012, exceto para CTe com numeração inutilizada (COD_SIT = 05).
...
documento, será comparado o CNPJ existente na CHV_CTE com o campo CNPJ do registro 0000, que corresponde ao
CNPJ do informante do arquivo, no caso de IND_EMIT = 0 (emissão própria). Será verificada a consistência da
informação do campo NUM_DOC e o número do documento contido na chave do CT-e. Será também comparada a UF
codificada na chave do CT-e com o campo UF informado no registro 0000.
Página 88 de 209
...
Se a legislação do ICMS definir que o imposto deve ser apropriado com base na data de prestção de serviços de transportes,
proceder da seguinte forma: todos os documentos de prestação de serviços com código de situação de documento igual a
“00” (documento regular) devem ser lançados no período de apuração informado no registro 0000, considerando a data da
...
trecho do frete. Neste caso, para subcontratação, o valor do campo pode ser qualquer um dos valores previstos.
...
Nível hierárquico - 3
Ocorrência - 1:N
Campo 01 - Valor Válido: [D195]
Campo 02 – Preenchimento: Informar o código da observação do lançamento.
Validação: o código informado deve constar do registro 0460.
Campo 03 - Preenchimento: utilizado para complementar a observação do lançamento fiscal, quando a descrição do
...
Nível hierárquico - 4
Ocorrência - 1:N
Campo 01 - Valor Válido: [D197]
Campo 02 - Validação: verifica se o COD_AJ está de acordo com a Tabela da UF do informante do arquivo.
Campo 04 - Preenchimento: pode ser informado se o ajuste/benefício for relacionado ao serviço constante na nota fiscal
de serviço de transporte (código 07) de saída. Para demais situações, não preencher.
...
Campo 02 - Valores válidos: [13, 14, 15, 16] – Ver tabela reproduzida na subseção 6.4 deste guia.
...
Campo 02 - Valores válidos: [2E, 13, 14, 15, 16] – Ver tabela reproduzida na subseção 6.4 deste guia..
...
Campo 02 - Validação: o valor informado deve existir no campo COD_PART do registro 0150.
Campo 03 - Valor Válido: [18] - – Ver tabela reproduzida na subseção 6.4 deste guia.
...
Campo 05 - Preenchimento: Série do RMD – Código 18.
Campo 06 - Preenchimento: Subsérie do RMD – Código 18.
Campo 07 - Validação: o valor informado no campo deve ser maior que “0” (zero).
...
Atualização: 13/03/2014
Campo 09 - Validação: o valor informado no campo deve ser maior que “0” (zero).
Campo 14 - Os contribuintes que entregarem a EFD-Contribuições relativa ao mesmo período de apuração do registro
...
Campo 02 - Valor Válido: [13, 14, 15, 16] - – Ver tabela reproduzida na subseção 6.4 deste guia.
Campo 05 - Validação: o valor informado no campo deve ser maior que “0” (zero), exceto se campo SER for igual “9999”
deste registro, quando este campo deve ser igual a zero.
Campo 06 - Validação: o valor informado no campo deve ser maior que “0” (zero).
...
Campo 03 - Valores Válidos: [0,1]
Campo 04 - Validação: o valor informado deve existir no campo COD_PART do registro 0150.
Campo 05 - Valores Válidos: [21, 22] – Ver tabela reproduzida na subseção 6.4 deste guia.
...
separadores de formatação.
Validação: o valor informado no campo deve ser menor ou igual ao valor no campo DT_FIN do registro 0000.
Campo 20 - Validação: o valor informado deve existir no campo COD_INF do registro 0450.
...
Campo 07 - Preenchimento: informar o valor total do item (equivalente à quantidade x preço unitário).
...
Campo 18 - Os contribuintes que entregarem a EFD-Contribuições relativa ao mesmo período de apuração do registro
0000 estão dispensados do preenchimento deste campo. Apresentar conteúdo VAZIO “||”.
...
separadores de formatação.
Validação: o valor informado no campo deve ser menor ou igual ao valor no campo DT_FIN do registro 0000.
Campo 04 - Preenchimento: informar a data em que foi encerrada a prestação de serviços, no formato “ddmmaaaa”, sem
separadores de formatação.
Validação: o valor informado no campo deve ser menor ou igual ao valor no campo DT_FIN do registro 0000.
...
conforme anexo do Convênio SN/70.
Campo 05 - Preenchimento: Na combinação de CST_ICMS, CFOP e ALIQ_ICMS, informar neste campo o valor dos
serviços e outras despesas acessórias, subtraído o desconto incondicional.
...
Campo 17 - Os contribuintes que entregarem a EFD-Contribuições relativa ao mesmo período de apuração do registro
0000 estão dispensados do preenchimento deste campo. Apresentar conteúdo VAZIO “||”.
Campo 18 - Os contribuintes que entregarem a EFD-Contribuições relativa ao mesmo período de apuração do registro
0000 estão dispensados do preenchimento deste campo. Apresentar conteúdo VAZIO “||”.
...
Campo 03 - Validação: o valor informado no campo deve existir na Tabela de Código Fiscal de Operação e Prestação,
conforme Anexo do Convênio SN/70.
Campo 06 - Validação: o valor constante neste campo deve corresponder à soma dos valores do campo BC_ICMS dos
...
Campo 08 - Validação: o valor constante neste campo deve corresponder à soma dos valores do campo VL_BC_ICMS
dos registros D610 (itens), que possuam a mesma combinação de valores dos campos CST, CFOP e Alíquota deste registro.
Campo 09 – Validação: o valor constante neste campo deve corresponder à soma dos valores do campo VL_ICMS dos
registros D610 (itens), que possuam a mesma combinação de valores dos campos CST, CFOP e Alíquota deste registro.
Campo 10 - Preenchimento: o valor deste campo só pode ser preenchido se os dois últimos dígitos do campo CST_ICMS
forem iguais a 20, 70 ou 90.
Validação: o valor informado neste campo deve ser maior que “0” (zero), se os dois últimos dígitos do campo CST_ICMS
...
emitidos para a UF informada no campo 02, incluída a parcela correspondente à unidade federada do prestador de serviços
(Convênio ICMS nº 52/05).
Campo 04 – Preenchimento: informar o somatório dos valores de ICMS correspondentes aos documentos emitidos para
assinantes da UF informada no campo 02. O valor efetivo de ICMS a recolher a cada uma das unidades federadas será
demonstrado nos registros de apuração.
Página 120 de 209
