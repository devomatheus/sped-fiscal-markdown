...
do registro 0200)
04 QTDE Quantidade, deste item da nota fiscal, que foi aplicada neste N - 05 O
bem, expressa na mesma unidade constante no documento fiscal
de entrada
05 UNID Unidade do item constante no documento fiscal de entrada C 6 - O
06 VL_ICMS_OP_AP Valor do ICMS da Operação Própria na entrada do item, N - 02 O
LICADO proporcional à quantidade aplicada no bem ou componente.
07 VL_ICMS_ST_AP Valor do ICMS ST na entrada do item, proporcional à N - 02 O
LICADO quantidade aplicada no bem ou componente.
08 VL_ICMS_FRT_A Valor do ICMS sobre Frete do Conhecimento de Transporte na N - 02 O
PLICADO entrada do item, proporcional à quantidade aplicada no bem ou
componente.
09 VL_ICMS_DIF_A Valor do ICMS Diferencial de Alíquota, na entrada do item, N - 02 O
PLICADO proporcional à quantidade aplicada no bem ou componente.
Observações:
Nível hierárquico - 5
Ocorrência - 1:N
...
06 – Para controle das mercadorias sujeitas ao regime
de substituição tributária – restituição/ ressarcimento/
complementação.
Observações:
Nível hierárquico - 2
Ocorrência – 1:N
...
06 – Para controle das mercadorias sujeitas ao regime de substituição tributária – restituição
/ressarcimento/ complementação – apresentado conforme legislação definida pela UF de domicílio do
contribuinte.
Validação: O arquivo que apresentar pelo menos um registro C180, C185, C330, C380, C430, C480, C815 ou
C870 deve possuir, pelo menos, um registro H005 com “MOT_INV” = 06 e com o campo “DT_INV” igual ao
dia imediatamente anterior ao campo “DT_INI” informado no registro 0000.
...
(controle das mercadorias sujeitas ao regime de substituição tributária – restituição/ ressarcimento/ complementação).
Nº Campo Descrição Tipo Tam Dec Obrig
Página 217 de 301
...
Campo 03 (VL_BC_ICMS_ST) -: Preenchimento: Informar o valor médio unitário da base de cálculo do ICMS/ST retido
por substituição tributária que tenha sido recolhido pelo próprio informante ou a título de reembolso que tenha sido suportado
por ele.
Campo 04 (VL_ICMS_ST) -: Preenchimento: Informar o valor médio unitário do ICMS/ST retido por substituição
tributária que tenha sido recolhido pelo próprio informante ou a título de reembolso que tenha sido suportado por ele. Quando
a mercadoria estiver sujeita, também, ao FCP adicionado ao ICMS/ST, neste campo deve ser informado o valor médio
unitário total da parcela do ICMS/ST + a parcela do FCP vinculado a este.
...
03 COD_ITE Código do item (campo 02 do Registro 0200) C 60 - O
M
...
02 COD_ITEM_DES Código do item de destino (campo 02 do C 060 - O
Registro 0200)
...
04 COD_DOC_OP Código de identificação da ordem de C 30 - OC
produção
...
05 COD_INS_SU Código do insumo que foi substituído, caso ocorra C 60 - OC
BST a substituição (campo 02 do Registro 0210)
Observações:
Nível hierárquico - 4
Ocorrência - 1:N
...
O reprocesso a ser escriturado no Registro K260 será aquele onde a quantidade produzida do produto a ser reprocessado já ti-
ver sido apontada no Registro K230 (entrada em estoque) e cujo código do produto reprocessado permaneça o mesmo do pro-
duto a ser reprocessado.
...
4 – correção de apontamento de reprocessamento/reparo
e/ou consumo relativo aos Registros K260/K265;
