...
A partir da vigência do Ajuste SINIEF 28/2021 e 39/2021 (01/12/2021) deixa de ser obrigatória a informação
...
CT-e e CT-e OS, emitidos por terceiros, (como por exemplo, o consórcio constituído nos termos do disposto nos arts. 278 e
279 da Lei nº 6.404, de 15 de dezembro de 1976), devem ser informados como emissão de terceiros, com o código de
...
14 CHV_CTE_REF Chave do Bilhete de Passagem Eletrônico substituído N 044* - OC OC
...
informar a chave do BP-e substituído. Nas demais situações o Campo não deve ser preenchido.
...
possuindo 7 dígitos. Campo obrigatório nas saídas para todos os modelos (a partir de 2022). Campo obrigatório nas entradas,
se “COD_MOD” do registro D100 for “57”, “63” ou “67”.
...
possuindo 7 dígitos. Campo obrigatório nas entradas, se “COD_MOD” do registro D100 for “57”, “63” ou “67”.
...
informado nos campos COD_MUN_ORIG e COD_MUN_DEST do Registro D100 forem iguais ou se um dos campos for
igual a 9999999 (transporte internacional).
...
Este registro tem por objetivo informar o complemento do Conhecimento de Transporte Rodoviário de Cargas (Código 08) e
...
4.3.1 da Nota Técnica, instituída pelo Ato COTEPE/ICMS nº 44/2018 e alterações, sendo que o primeiro caractere sempre
será Zero.
...
TERCEIRO CARACTERE do Código do Ajuste (Tabela 5.3 da Nota Técnica, instituída pelo Ato COTEPE/ICMS nº
...
no campo 07 - VL_ICMS gera um ajuste a crédito (campo 07- VL_AJ_CREDITOS) no registro E110 e
...
Guia Prático EFD-ICMS/IPI – Versão 3.1.5
Atualização: 24 de agosto de 2023
no campo 07 - VL_ICMS gera um ajuste a débito (campo 03- VL_AJ_DEBITOS) no registro E110 e
...
b) se os dois últimos dígitos deste campo forem diferentes de 30, 40, 41, 50, e 60, então os valores
dos campos VL_BC_ICMS, ALIQ_ICMS e VL_ICMS deverão ser maiores que “0” (zero);
...
alterações, que discrimina os códigos dos Totalizadores Parciais da REDUÇÃO Z, prevista também na subseção 6.6 deste
guia.
...
ou “xxTnnnn”, o valor deste campo deve ser igual à soma do campo VL_BC_ICMS do registro D390 e também deve ser
igual à soma do campo VL_SERV do registro D370.
...
Tabela B constante no Anexo do Convênio SN/70.
Validação: o valor informado no campo deve existir na Tabela da Situação Tributária referente ao ICMS, constante do
Artigo 5º Anexo do Convênio SN/70 e obedecer às seguintes regras:
ICMS Normal:
se os dois últimos dígitos deste campo forem 30, 40, 41, 50, ou 60, então os valores dos campos VL_BC_ICMS,
ALIQ_ICMS e VL_ICMS deverão ser iguais a “0” (zero);
...
02 UF Sigla da unidade da federação C 002* - apresentar O
Página 211 de 360

--- Página 212 ---
Guia Prático EFD-ICMS/IPI – Versão 3.1.5
Atualização: 24 de agosto de 2023
03 VL_BC_ICMS Valor da base de cálculo do ICMS N - 02 O
...
Este registro tem por objetivo escriturar notas de comunicação individualizadas, de entradas e saídas. As notas
normais de saída - emitidas com a tag finalidade de emissão com valor 0 - devem ser escrituradas neste registro quando não
for obrigatório escriturá-las de forma consolidada.
...
Nas entradas, a informação dos campos VL_DOC, VL_DESC, VL_SERV, VL_SERV_NT, VL_TERC e VL_DA
não deve levar em conta os serviços com cClass 110, 120 e 130.
...
A NFCom que contenha apenas itens sem a indicação de Código de Situação Tributária – CST não deve ser
escriturada.
Exceção 1: Para as Notas Fiscais emitidas por regime especial ou norma específica (campo COD_SIT igual a “08”), no
registro D700, somente os campos REG, IND_OPER, IND_EMIT, COD_PART (nas entradas), COD_MOD, COD_SIT,
SER, NUM_DOC e DT_DOC são obrigatórios. Os demais campos são facultativos (se forem preenchidos, serão validadas e
aplicadas as regras de campos existentes). No registro D730 deverão ser observados os campos obrigatórios.
0.
...
10 DT_E_S Data da entrada N 008* - OC -
...
de ICMS.
15 VL_TERC Valores cobrados em nome de terceiros.
...
27 HASH_DOC_REF do registro do Arquivo tipo mestre de documento C 32 - OC OC
fiscal, conforme definido no Convênio 115/2003.
...
referenciado.
Código do município do destinatário conforme a
...
separadores de formatação.
Validação: o valor informado no campo deve ser menor ou igual ao valor no campo DT_FIN do registro 0000.
Campo 10 (DT_E_S) - Preenchimento: informar a data da entrada ou saída da prestação do serviço, no formato
“ddmmaaaa”, sem separadores de formatação.
Validação: o valor informado no campo deve ser menor ou igual ao valor no campo DT_FIN do registro 0000.
...
registro 0000 estão dispensados do preenchimento deste campo. Apresentar conteúdo VAZIO “||”.
Campo 21 (VL_COFINS) - Os contribuintes que entregarem a EFD-Contribuições relativa ao mesmo período de apuração
do registro 0000 estão dispensados do preenchimento deste campo. Apresentar conteúdo VAZIO “||”.
...
Validação: deve ser informado quando o campo FIN_DOCe for igual a “3 - Substituição”. Não informar nas demais
situações.
...
Validação: obrigatório quando COD_MOD_DOC_REF for igual a “62”. Será conferido o dígito verificador (DV) da chave
do documento eletrônico.
Campo 27 (HASH_DOC_REF) - Preenchimento: deve ser preenchido com o código de autenticação digital do registro,
campo 36 do registro do Arquivo tipo mestre de documento fiscal, conforme definido no Convênio 115/2003, se o documento
...
COD_MOD_DOC_REF for diferente de “21” e “22”. Quando COD_MOD_DOC_REF for igual a “21” ou “22”, é
obrigatória a informação simultânea de SER_DOC_REF, NUM_DOC_REF e MES_DOC_REF, ou a informação de
HASH_DOC_REF. Se HASH_DOC_REF for informado, este campo deve ter conteúdo VAZIO “||”.
...
Relativamente às Notas Fiscais Fatura de Serviços de Comunicação Eletrônica (NFCom), não devem ser
apresentados neste registro os itens sem a indicação de Código de Situação Tributária – CST
Validação do Registro: não podem ser informados dois ou mais registros com a combinação de mesmos valores
dos campos CST_ICMS, CFOP e ALIQ_ICMS para o mesmo documento.
Nº Campo Descrição Tipo Tam Dec Entr. Saídas
...
tabela indicada no item 4.2.2
04 ALIQ_ICMS Alíquota do ICMS N 006 02 OC OC
05 VL_OPR Valor da prestação correspondente à combinação de N - 02 O O
CST_ICMS, CFOP, e alíquota do ICMS, incluídas as
despesas acessórias e acréscimos
06 VL_BC_ICMS Parcela correspondente ao "Valor da base de cálculo N - 02 O O
do ICMS" referente à combinação CST_ICMS, CFOP,
e alíquota do ICMS
07 VL_ICMS Parcela correspondente ao "Valor do ICMS" referente N - 02 O O
à combinação CST_ICMS, CFOP, e alíquota do
ICMS, incluindo o FCP, quando aplicável, referente à
combinação de CST_ICMS, CFOP e alíquota do
ICMS.
08 VL_RED_BC Valor não tributado em função da redução da base de N - 02 O O
cálculo do ICMS, referente à combinação de
...
CARACTERE do Código do Ajuste (Tabela 5.3 da Nota Técnica, instituída pelo Ato COTEPE/ICMS nº 44/2018 e
alterações).
...
1. o terceiro caractere do código de ajuste (tabela 5.3) do reg. C737 for igual a “2 – Estorno de Débito”
...
15 VL_PIS Valor total do PIS N - 02 - O
16 VL_COFINS Valor total da COFINS N - 02 - O
Observações:
Nível hierárquico - 2
Ocorrência – 1:N
Campo 01 (REG) - Valor Válido: [D750]
Campo 02 (COD_MOD) - Valores Válidos: [62] – Ver tabela reproduzida na subseção 1.4 deste guia.
Campo 07 (VL_DOC) – Preenchimento: corresponde à soma dos campos VL_SERV, VL_SERV_NT, VL_TERC e
VL_DA subtraído do campo VL_DESC.
Página 219 de 360
...
do registro 0000 estão dispensados do preenchimento deste campo. Apresentar conteúdo VAZIO “||”.
REGISTRO D760: REGISTRO ANALÍTICO DA ESCRITURAÇÃO CONSOLIDADA DA
...
ICMS, constante do Artigo 5º do Convênio SN/70.
Campo 03 (CFOP) - Validação: o valor informado no campo deve existir na Tabela de Código Fiscal de Operação e
Prestação, conforme Ajuste SINIEF 07/01. Os valores de CFOP informados poderão ser iniciados com 5, 6 ou 7.
Campo 05 (VL_OPR) - Preenchimento: Na combinação de CST_ICMS, CFOP e ALIQ_ICMS, informar neste campo (8)
VL_SERV somado as outras despesas acessórias, campo (12) VL_DA e subtraídos do campo (11) VL_DESC, desconto
incondicional.
Campo 06 (VL_BC_ICMS) - Preenchimento: informar a base de cálculo do ICMS, referente à combinação dos campos
CST_ICMS, CFOP e ALIQ_ICMS deste registro.
Campo 07 (VL_ICMS) - Preenchimento: informar o valor do ICMS referente à combinação dos campos CST_ICMS,
