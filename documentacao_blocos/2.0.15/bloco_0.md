...
Para as SEFAZ que exigem a informação do hash do arquivo, trata-se da hash do arquivo RETIFICADOR
assinado. Para obtê-lo utilizar a opção “Dados da Escrituração” da aba Relatórios, a hash está identificado com o nome “ID
...
nº 7.212/2010, ainda que seja uma indústria, deve informar a opção "1 - Outras" no campo IND_ATIV do registro 0000. Se
...
neste registro.
Obs.: Não devem ser informados como participantes os CNPJ e CPF apenas citados nos registros C350
– Nota Fiscal de Venda ao Consumidor, C460 – Documento Fiscal emitido por ECF e no C100,
quando se tratar de NFC-e - Nota Fiscal Eletrônica ao Consumidor Final - modelo 65.
...
Validação: quando se tratar de alterações nos campos CNPJ ou CPF, referidos campos serão validados
conforme as regras de verificação de dígitos verificadores. Se o NR_CAMPO = 08 (COD_MUN) será verificada sua
...
produto, se houver
05 COD_ANT_ITEM Código anterior do item com relação à última informação C 060 - N
apresentada. (informar
no 0205)
...
possuir mais de um tipo de item (destinação), deve ser informado o tipo de maior relevância.
Página 24 de 213
...
(industrial ou equiparado a industrial), mas apenas para os itens correspondentes à atividade fim ou quando ge rarem
...
Campo 02 (COD_COMB) - Preenchimento: utilizar o código do combustível, conforme Tabela de Produtos para
...
consumida por unidade do item composto, considerando-se apenas a perda normal do processo industrial;
